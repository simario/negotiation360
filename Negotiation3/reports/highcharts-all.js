...
(function() {
		...
		attr: function(a, b) {
			...
			if (ga(a)) c = a, h === "circle" ? c = {
				...
				for (c in a) if (j = !1, d = a[c], e = k[c] && k[c].call(this, d, c), e !== !1) {
					...
					else if (c === "x" && h === "text") for (e = 0; e < g.childNodes.length; e++) f = g.childNodes[e], z(f, "x") === z(g, "x") && z(f, "x", d);
					...
					if (l && /^(width|height|visibility|x|y|d|transform|cx|cy|r)$/.test(c)) for (e = l.length; e--;) z(l[e], c, c === "height" ? t(d - (l[e].cutHeight || 0), 0) : d);
	...
	if (!X && !ca) {
		Q.VMLElement = da = {
			...
			pathToVML: function(a) {
				for (var b = a.length, c = []; b--;) if (ya(a[b])) c[b] = v(a[b] * 10) - 5;
				else if (a[b] === "Z") c[b] = "x";
	...
	ka.prototype = {
		...
		defaultLabelFormatter: function() {
			...
			else if (f && a >= 1E3) for (; f-- && g === u;) c = Math.pow(1E3, f + 1), a >= c && e[f] !== null && (g = Ga(b / c, -1) + e[f]);
	...
	M.prototype = {
			...
			if (d !== !1 && h && g === h && !e.cropped && !e.hasGroupedData) p(a, function(a, b) {
				...
				if (q && h > q) {
					...
					if (ya(k)) {
						...
						e.xIncrement = m
					} else if (La(k)) if (s) for (c = 0; c < h; c++) i = a[c], t[c] = i[0], r[c] = i.slice(1, s + 1);
					else for (c = 0; c < h; c++) i = a[c], t[c] = i[0], r[c] = i[1];
					else oa(12)
		...
		translate: function() {
			...
			this.generatePoints();
			for (var a = this.options, b = a.stacking, c = this.xAxis, d = c.categories, e = this.yAxis, f = this.points, g = f.length, h = !! this.modifyValue, i = a.pointPlacement, j = i === "between" || ya(i), k = a.threshold, a = 0; a < g; a++) {
				var l = f[a],
					m = l.x,
					n = l.y,
					q = l.low,
		...
		drawPoints: function() {
			...
			if (l.enabled || this._hasPointMarkers) for (f = b.length; f--;) if (g = b[f], d = S(g.plotX), e = g.plotY, k = g.graphic, i = g.marker || {}, a = l.enabled && i.enabled === u || i.enabled, n = c.isInsidePlot(v(d), e, c.inverted), a && e !== u && !isNaN(e) && g.y !== null) if (a = g.pointAttr[g.selected ? "select" : ""] || m, h = a.r, i = o(i.symbol, this.symbol), j = i.indexOf("url") === 0, k) k.attr({
			...
			for (H = 2; H--;) {
				...
				if (l > 0) {
					...
					for (A = 0; A < G; A++) {
						...
						if (C < A && b[A] !== null) C = A;
						else for (u < G - A + C && b[A] !== null && (C = u - G + A); b[C] === null;) C++;
						I.push({
							i: C,
							y: b[C]
						});
...
(function(l, C) {
		...
		M = {
			...
			getPlotLinePath: function(a, b) {
				var c = this.center,
					...
					g, e, k;
				this.isCircular ? k = ["M", c[0] + d.plotLeft, c[1] + d.plotTop, "L", f.x, f.y] : this.options.gridLineInterpolation === "circle" ? (a = this.translate(a)) && (k = this.getLinePath(0, a)) : (g = d.xAxis[0], k = [], a = this.translate(a), c = g.tickPositions, g.autoConnect && (c = c.concat([c[0]])), b && (c = [].concat(c).reverse()), t(c, function(c, b) {
					e = g.getPosition(c, a);
	...
	r(F, "init", function(a, b, c) {
		...
		var m = c.pane || 0;
		if (d) {
			if (y(this, e ? V : M), k = !g) this.defaultRadialOptions = this.defaultRadialGaugeOptions
		...
		if (!e && (d || f)) {
			...
			this.endAngleRad = n = (q(m.endAngle, m.startAngle + 360) - 90) * Math.PI / 180;
	...
	r(z, "getLabelPosition", function(a, b, c, d, f, g, e, k, n) {
		...
		m.isRadial ? (a = m.getPosition(this.pos, m.center[2] / 2 + q(g.distance, -25)), g.rotation === "auto" ? d.attr({
			rotation: h
		}) : i === null && (i = m.chart.renderer.fontMetrics(d.styles.fontSize).b - d.getBBox().height / 2), j === null && (j = m.isCircular ? h > 20 && h < 160 ? "left" : h > 200 && h < 340 ? "right" : "center" : "center", d.attr({
			align: j
	...
	h.boxplot = x(h.column, {
		...
		drawPoints: function() {
			...
			t(b, function(b) {
				...
				u = {};
				y = b.color || a.color;
				if (b.plotY !== C) if (f = b.pointAttr[b.selected ? "selected" : ""], x = z.width, s = A(z.x), v = s + x, w = D(x / 2), g = A(E ? b.q1Plot : b.lowPlot), e = A(E ? b.q3Plot : b.lowPlot), k = A(b.highPlot), n = A(b.lowPlot), p.stroke = b.stemColor || c.stemColor || y, p["stroke-width"] = q(b.stemWidth, c.stemWidth, c.lineWidth), p.dashstyle = b.stemDashStyle || c.stemDashStyle, o.stroke = b.whiskerColor || c.whiskerColor || y, o["stroke-width"] = q(b.whiskerWidth, c.whiskerWidth, c.lineWidth), u.stroke = b.medianColor || c.medianColor || y, u["stroke-width"] = q(b.medianWidth, c.medianWidth, c.lineWidth), u["stroke-linecap"] = "round", i = p["stroke-width"] % 2 / 2, j = s + w + i, l = ["M", j, e, "L", j, k, "M", j, g, "L", j, n, "z"], E && (i = f["stroke-width"] % 2 / 2, j = A(j) + i, g = A(g) + i, e = A(e) + i, s += i, v += i, H = ["M", s, e, "L", s, g, "L", v, g, "L", v, e, "L", s, e, "z"]), B && (i = o["stroke-width"] % 2 / 2, k += i, n += i, r = ["M", j - w * B, k, "L", j + w * B, k, "M", j - w * B, n, "L", j + w * B, n]), i = u["stroke-width"] % 2 / 2, m = D(b.medianPlot) + i, I = ["M", s, m, "L", v, m, "z"], h) b.stem.animate({
					d: l
	...
	h.waterfall = x(h.column, {
		...
		translate: function() {
			...
			for (d = 0, c = f.length; d < c; d++) {
				...
				g.isSum || g.isIntermediateSum ? (e.y = b.translate(i[1], 0, 1), e.height = b.translate(i[0], 0, 1) - e.y) : m += k.total;
	...
	L.prototype.beforePadding = function() {
		...
		}), p.length && h > 0 && q(this.options.min, this.userMin) === C && q(this.options.max, this.userMax) === C && (f -= b, l *= (b + d - f) / b, this.min += d / l, this.max += f / l))
	};
	(function() {
		function a(a, b, c) {
			a.call(this, b, c);
			...
			if (c.polar) {
				if (c.renderer.isSVG) if (d === !0 && (d = {}), b) {
					if (c = {
						...
						scaleX: 0.001,
						scaleY: 0.001
					}, f.attr(c), i) i.attrSetters = f.attrSetters, i.attr(c)
				} else c = {
					translateX: h,
					translateY: l,
					scaleX: 1,
		...
		h.spline && r(h.spline.prototype, "getPointSpline", function(a, b, c, d) {
			...
			if (this.chart.polar) {
				...
				if (a && j) h = a.plotX, l = a.plotY, b = j.plotX, p = j.plotY, h = (1.5 * f + h) / 2.5, l = (1.5 * i + l) / 2.5, j = (1.5 * f + b) / 2.5, o = (1.5 * i + p) / 2.5, b = Math.sqrt(Math.pow(h - f, 2) + Math.pow(l - i, 2)), p = Math.sqrt(Math.pow(j - f, 2) + Math.pow(o - i, 2)), h = Math.atan2(l - i, h - f), l = Math.atan2(o - i, j - f), o = Math.PI / 2 + (h + l) / 2, Math.abs(h - o) > Math.PI / 2 && (o -= Math.PI), h = f + Math.cos(o) * b, l = i + Math.sin(o) * b, j = f + Math.cos(Math.PI + o) * p, o = i + Math.sin(Math.PI + o) * p, c.rightContX = j, c.rightContY = o;
		...
		if (h.column) f = h.column.prototype, r(f, "animate", b), r(f, "translate", function(a) {
			var b = this.xAxis,
				c = this.yAxis.len,
				d = b.center,
				f = b.startAngleRad,
...
(function(j) {
	...
	j.extend(n.prototype, {
		...
		parseTypes: function() {
			for (var b = this.columns, a = b.length, c, d, e, h; a--;) for (c = b[a].length; c--;) d = b[a][c], e = parseFloat(d), h = this.trim(d), h == e ? (b[a][c] = e, e > 31536E6 ? b[a].isDatetime = !0 : b[a].isNumeric = !0) : (d = this.parseDate(d), a === 0 && typeof d === "number" && !isNaN(d) ? (b[a][c] = d, b[a].isDatetime = !0) : b[a][c] = h === "" ? null : h)
		},
		dateFormats: {
			"YYYY-mm-dd": {
		...
		parseDate: function(b) {
			var a = this.options.parseDate,
				c, d, e;
			a && (c = a(b));
			if (typeof b === "string") for (d in this.dateFormats) a = this.dateFormats[d], (e = b.match(a.regex)) && (c = a.parser(e));
			return c
		},
